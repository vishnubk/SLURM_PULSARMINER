    ...
    if [ -d "$dir" ] && [[ $(basename $dir) == cf* || $(basename $dir) == if* ]]; then
        ...
        for model in $ml_model_dir*.pkl; do
            ...
            if [ ! -f $fold_dir/$pics_results ]; then
                singularity exec -H $HOME:/home1 -B /hercules/:/hercules/ /hercules/scratch/vishnu/singularity_images/trapum_pulsarx_fold_docker_20220411.sif python pics_classifier.py -i $fold_dir/ -m $absolute_model_path -o $results_dir_high_score
            fi
        done
    fi
done
