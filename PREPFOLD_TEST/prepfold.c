...
int main(int argc, char *argv[])
{
    ...
    if (RAWDATA || insubs) {
        ...
        if (RAWDATA) {
            ...
            printf("Number of samples in file = %lld\n", s.N);
            printf("Number of samples per record = %d\n", ptsperrec);
            printf("Number of channels in file = %lld\n", numrec);
            if (!cmd->nsubP) {
                cmd->nsub = 1;  // flag
                if (numchan <= 256) {
                    if (numchan % 32 == 0) {
    ...
    if (RAWDATA || insubs) {
        ...
        printf("Number of records to use before  = %lld\n", numrec);
        ...
        //printf("Number of records in file = %lld\n", N);
        printf("Number of records to use   = %lld\n", numrec);
        printf("DBLCORRECT                 = %f\n", DBLCORRECT);
        printf("startT                     = %f\n", cmd->startT);
        printf("endT                       = %f\n", cmd->endT);
        printf("lorec                      = %lld\n", lorec);
    ...
    if (cmd->psrnameP) {
        ...
        printf("fd = %f\n", fd);
        printf("fdd = %f\n", fdd);
        printf("Before switch_f_and_p\n");
        printf("search.bary.p1 = %f\n", search.bary.p1);
        printf("search.bary.p2 = %f\n", search.bary.p2);
        printf("search.bary.p3 = %f\n", search.bary.p3);
        ...
        {   
            printf("RAWDATA || insubs\n");
            printf("Before operation:\n");
            printf("f = %f, Hex: %a\n", f, f);
            printf("lorec = %ld, Hex: %a\n", lorec, lorec);
            printf("recdt = %f, Hex: %a\n", recdt, recdt);
            printf("fd = %f, Hex: %a\n", fd, fd);

            f += lorec * recdt * fd;
        
            printf("After lorec = %ld\n", lorec);
            ...
            // f += lorec * recdt * fd;
            // printf("Address of f after: %p\n", (void *)&f);
            // printf("f after operation: %f\n", f);
            // printf("a = %f\n", a);
            // printf("b = %f\n", b);
        ...
        {   printf("NOT RAWDATA || insubs\n");
            f += lorec * search.dt * fd;
        }
        if (rzwidata.bary)
        {
            printf("Barycentric data\n");
            printf("f = %f\n", f);
            printf("fd = %f\n", fd);
            printf("fdd = %f\n", fdd);
            printf("search.bary.p1 = %f\n", search.bary.p1);
