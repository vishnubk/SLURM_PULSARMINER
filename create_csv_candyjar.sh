...
for beam in "$cluster/$epoch"/{cf,if}*; do
 ...
 #  echo $sing_command python get_csv_from_pfds.py -pfds "$beam"/05_FOLDING/"$cluster"_"$epoch"_"$beam_name"/*.pfd -meta "$meta_file" -beam_name "$beam_name" -utc "$utc_time" -out "$output_file" 
  fi
done
